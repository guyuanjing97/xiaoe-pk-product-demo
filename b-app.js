(() => {
  ...
  const defaultActivity=()=>({
    id:'pk-'+Date.now(),name:'',cover:'',description:'',mode:'1v1PK',teamSize:3,
    questionCount:0,seconds:15,nextIntervalSeconds:2,baseScore:10,wrongScore:-2,
  ...
  function pkQuickSettings(a){
    return `<div class="form-row"><label class="form-label">PK类型</label><div class="radio-row"><label><input type="radio" name="mode" value="1v1PK" ${a.mode!=='组队PK'?'checked':''}>1v1PK</label><label><input type="radio" name="mode" value="组队PK" ${a.mode==='组队PK'?'checked':''}>组队PK</label></div></div>
      ...
      <div class="form-row"><label class="form-label">计分规则</label><div class="two-fields"><span>答对</span><div class="unit-field"><input class="input" id="baseScore" type="number" min="1" value="${a.baseScore||10}"><span>分</span></div></div></div>
  ...
  function collectEditable(a){
    ...
    a.seconds=Number(val('#seconds')||a.seconds);a.nextIntervalSeconds=Number(val('#nextIntervalSeconds')||a.nextIntervalSeconds);a.baseScore=Number(val('#baseScore')||a.baseScore);a.wrongScore=Number(val('#wrongScore')??a.wrongScore);
    a.teamSize=Number(val('#teamSize')||a.teamSize);a.attempts=Number(val('#attempts')||a.attempts);
  ...
  function usersPanel(a){
    ...
    const rows=learners.filter(x=>(dept==='全部'||x.department===dept)&&(participation==='全部'||(participation==='已参赛'?x.matches>0:x.matches===0))&&(!term||x.name.includes(term)||x.account.includes(term)||x.phone.includes(term))).slice().sort((x,y)=>y.score-x.score);
    return `<div class="filter-strip user-filters"><label>关键词：<select id="userKey"><option>姓名</option><option>账号</option><option>手机号</option></select></label><input id="userSearch" placeholder="请输入姓名、账号或手机号" value="${escapeHtml(state.userSearch)}"><label>所在部门：<select id="userDepartment"><option>全部</option>${['产品中心','客户成功部','销售中心'].map(x=>`<option ${x===dept?'selected':''}>${x}</option>`).join('')}</select></label><label>参赛状态：<select id="userParticipation">${['全部','已参赛','未参赛'].map(x=>`<option ${x===participation?'selected':''}>${x}</option>`).join('')}</select></label><button class="btn primary" data-filter-users>筛选</button><button class="btn" data-export-users>导出</button><button class="btn text" data-reset-users>重置筛选条件</button></div><div class="toolbar">${a.visibility==='部分学员'?'<button class="btn primary" data-add-users>添加学员</button>':''}<button class="btn">联系用户⌄</button><button class="btn">贴标签⌄</button><span class="spacer"></span><span>按总成绩分数从高到低排序，共${rows.length}条</span></div><table class="data-table"><thead><tr><th><input type="checkbox"></th><th>学员</th><th>账号绑定手机号</th><th>姓名</th><th>所在部门</th><th>总成绩分数</th><th>参赛次数</th><th>最近参赛时间</th></tr></thead><tbody>${rows.map(x=>`<tr><td><input type="checkbox"></td><td><span class="avatar"></span><button class="link">${x.account}</button></td><td>${x.phone}</td><td>${x.name}</td><td>${x.department}</td><td>${x.score}</td><td>${x.matches}</td><td>${x.last}</td></tr>`).join('')}</tbody></table>`
  }
  function dataPanel(a){
    ...
    const scoreHint=`<div class="score-hint">成绩默认按照参加该 PK 赛学员的总得分从高到低排序，不支持筛选。若需多维分析，可导出列表字段到 Excel 自行处理。</div>`;
    const scoreActions=`<div class="toolbar analysis-actions"><button class="btn" data-export-score>导出成绩排名</button><span class="help">可选择导出列表中的多个字段</span></div>`;
    const otherFilter=`<div class="filter-strip analysis-filter"><label>关键词：<select><option>姓名</option></select></label><input type="text" placeholder="请输入姓名"><label>对局时间：<input type="date"><span>至</span><input type="date"></label><button class="btn primary">筛选</button><button class="btn" data-export-matches>导出对局记录</button><button class="btn text">重置筛选条件</button></div>`;
    return `<div class="panel-head"><h2>PK赛概况</h2><span class="spacer"></span><button class="btn text" data-toggle-overview>${state.analysisOverviewHidden?'显示概况':'隐藏概况'}</button></div>${overview}<div class="tabs">${[['score','成绩排名'],['matches','对局记录'],['questions','题目分析']].map(([id,label])=>`<button class="${state.dataTab===id?'active':''}" data-data-tab="${id}">${label}</button>`).join('')}</div>${rankingSwitch}${state.dataTab==='score'?scoreActions+scoreHint:otherFilter}${dataTable(a,state.dataTab,scoreRows,hasData)}`
  }
  function dataTable(a,tab,rows,hasData){
    const empty=(cols,text)=>`<tr><td colspan="${cols}" class="table-empty">${text}</td></tr>`;
    if(tab==='score'&&state.rankMode==='team'&&a.mode==='组队PK')return `<table class="data-table"><thead><tr><th>排名</th><th>部门/队伍</th><th>总成绩分数</th><th>参赛人数</th><th>对局数</th><th>胜场</th><th>胜率</th><th>对局时间</th><th>操作</th></tr></thead><tbody>${hasData?['产品中心','客户成功部','销售中心'].map((x,i)=>`<tr><td>${i+1}</td><td>${x}</td><td>${[368,352,346][i]}</td><td>${[31,28,27][i]}</td><td>${[89,82,77][i]}</td><td>${[52,46,40][i]}</td><td>${[58.4,56.1,51.9][i]}%</td><td>${['2026-08-13 16:20','2026-08-13 15:48','2026-08-13 15:12'][i]}</td><td><button class="btn text">查看详情</button></td></tr>`).join(''):empty(9,'产生对局后生成团队成绩')}</tbody></table>`;
    if(tab==='score')return `<table class="data-table"><thead><tr><th>排名</th><th>姓名</th><th>部门</th><th>总成绩分数</th><th>参赛场次</th><th>胜场</th><th>胜率</th><th>正确率</th><th>平均答题用时</th><th>对局时间</th><th>操作</th></tr></thead><tbody>${rows.length?rows.map((x,i)=>`<tr><td>${i+1}</td><td><button class="link">${x.name}</button></td><td>${x.department}</td><td>${x.score}</td><td>${x.matches}</td><td>${x.wins}</td><td>${(x.wins/x.matches*100).toFixed(1)}%</td><td>${x.accuracy}</td><td>${x.avgTime}</td><td>${x.last}</td><td><button class="btn text">查看详情</button></td></tr>`).join(''):empty(11,'产生对局后生成成绩排名')}</tbody></table>`;
    if(tab==='matches'){
  ...
  function openScoreExport(){
    const fields=['排名','姓名','部门','总成绩分数','参赛场次','胜场','胜率','正确率','平均答题用时','对局时间'];
    const selected=new Set(['姓名','部门','总成绩分数','参赛场次','胜场']);
    modalRoot.innerHTML=`<div class="modal"><div class="dialog small"><div class="dialog-head"><h3>导出成绩排名</h3><button data-close>×</button></div><div class="dialog-body system-count"><p>选择需要导出的列表字段，可多选。</p>${fields.map(f=>`<label class="check-row"><input type="checkbox" data-export-field="${f}" ${selected.has(f)?'checked':''}>${f}</label>`).join('')}</div><div class="dialog-foot"><button class="btn" data-close>取消</button><button class="btn primary" data-export-confirm>导出</button></div></div></div>`;
    modalRoot.querySelectorAll('[data-close]').forEach(x=>x.onclick=()=>modalRoot.innerHTML='');
    modalRoot.querySelectorAll('[data-export-field]').forEach(x=>x.onchange=()=>{x.checked?selected.add(x.dataset.exportField):selected.delete(x.dataset.exportField)});
    modalRoot.querySelector('[data-export-confirm]').onclick=()=>{if(!selected.size)return toast('请至少选择一个字段');modalRoot.innerHTML='';toast(`已导出：${[...selected].join('、')}`)}
  }
